...
/**
 * Thread-safe class that functions as a limited-size buffer of objects. This works like [XThreadStringBuffer]
 * but handles arbitrary objects, and [XThreadStringBuffer] could in fact be implemented as a strongly typed
 * extension of this. Could move this to ManifoldCF core, and update XThreadStringBuffer to match for backward
 * compatibility.
 */
public class XThreadObjectBuffer<T>
{
  protected static int MAX_SIZE = 1024;

  protected List<T> buffer = new ArrayList<>(MAX_SIZE);

  protected boolean complete = false;
  protected boolean abandoned = false;

  /** Constructor */
  public XThreadObjectBuffer()
  {
  }

  /** Add an object to the buffer, and block if the buffer is full */
  public synchronized void add(T string)
  ...
  */
  ...
  /**
   * Pull an Object off the buffer, and wait if there's more to come.
   * Called by the receiving thread!
